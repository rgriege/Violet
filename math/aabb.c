...
b8 aabb_contains_v(const aabb * box, const v2 * point)
{
...
void aabb_transform(aabb * box, const m3 mat)
{
	...
	m3_mul(mat, &box->top_left, array_append_null(&p.vertices));
	m3_mul(mat, &top_right, array_append_null(&p.vertices));
	m3_mul(mat, &box->bottom_right, array_append_null(&p.vertices));
	m3_mul(mat, &bottom_left, array_append_null(&p.vertices));
