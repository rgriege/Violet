...
void aabb_from_dims(aabb * box, const r32 left, const r32 top, const r32 right, const r32 bottom)
{
	box->top_left.x = left;
	box->top_left.y = top;
...
b8 aabb_contains_v(const aabb * box, const v2 * point) const
{
...
b8 aabb_contains_b(const abbb * lhs, const aabb * rhs) const
{
	...
	return interval_contains(lhs_x, rhs_x) && interval_contains(lhs_y, rhs_y);
}

b8 aabb_overlaps(const aabb * lhs, const aabb * rhs) const
{
	interval lhs_x = { lhs->top_left.x, lhs->bottom_right.x };
	interval rhs_x = { rhs->top_left.x, rhs->bottom_right.x };
	interval lhs_y = { lhs->top_left.y, lhs->bottom_right.y };
...
void aabb_extend_v(aabb * box, const v2 * point)
{
	aabb_from_dims(box,
...
void aabb_extend_b(aabb * lhs, const aabb * rhs)
{
	aabb_extend_v(lhs, &rhs->top_left);
	aabb_extend_v(lhs, &rhs->bottom_right);
}

void aabb_translate(aabb * box, const v2 * off)
{
	v2_add(&box->top_left, off, &box->top_left);
	v2_add(&box->top_right, off, &box->top_right);
}
...
void aabb_transform(aabb * box, const m3 * mat)
{
	...
	m3_mul(mat, &box->top_left, array_add(p.vertices));
	m3_mul(mat, &top_right, array_add(p.vertices));
	m3_mul(mat, &box->bottom_right, array_add(p.vertices));
	m3_mul(mat, &bottom_left, array_add(p.vertices));

	poly_get_bounding_box(&p, box);
}

void aabb_get_center(const aabb * box, v2 * center)
{
	center->x = (box->bottom_right.x - top_left.x) / 2.f;
	center->y = (box->top_left.y - bottom_right.y) / 2.f;
}
