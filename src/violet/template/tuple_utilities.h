#ifndef VIOLET_TUPLE_UTILITIES_H
#define VIOLET_TUPLE_UTILITIES_H
	...
	namespace detail
	{
		...
		struct extractor
		{
			static void extract(const InTuple & inTuple, OutTuple & outTuple)
			{
				...
				std::get<OutWidth - 1>(outTuple) = vlt::get<typename std::tuple_element<OutWidth - 1, OutTuple>::type>(inTuple);
			}
		};
...
namespace std
{
	...
	struct hash<std::tuple<Args...>>
	{
		...
		{
			size_t seed = 0;
			vlt::detail::hash_combine(seed, tuple, vlt::detail::Index<sizeof...(Args)-1>());
			return seed;
		}
	};
}
